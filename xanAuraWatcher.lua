...
local f = CreateFrame("frame","xanAuraWatcher",UIParent)
...
function f:PLAYER_LOGIN()
	...
	self:CreateFrames()
	self:RestoreLayout("xanAuraWatcher")
	...
	SLASH_XANAURAWATCHER1 = "/xanaw";
	SlashCmdList["XANAURAWATCHER"] = xanAW_SlashCommand;
	
	local ver = GetAddOnMetadata("xanAuraWatcher","Version") or '1.0'
	DEFAULT_CHAT_FRAME:AddMessage(string.format("|cFF99CC33%s|r [v|cFFDF2B2B%s|r] loaded:   /xanaw", "xanAuraWatcher", ver or "1.0"))
...
function xanAW_SlashCommand(cmd)

	local a,b,c=strfind(cmd, "(%S+)"); --contiguous string of non-space characters
	
	if a then
		if c and c:lower() == "lock" then
			if not InCombatLockdown() and _G["XAW_Anchor"] then
				if _G["XAW_Anchor"]:IsVisible() then
					_G["XAW_Anchor"]:Hide()
				else
					_G["XAW_Anchor"]:Show()
				end
		...
		elseif c and c:lower() == "reset" then
			DEFAULT_CHAT_FRAME:AddMessage("xanAuraWatcher: Frame position has been reset!");
		...
		elseif c and c:lower() == "size" then
			if b then
				...
				if sizenum and sizenum ~= "" and tonumber(sizenum) then
					XanAW_DB.size = tonumber(sizenum)
					ReloadUI()
					return true
				end
			end
...
function f:CreateFrames()
	...
	local sWdith = XanAW_DB.size
	local sHeight = XanAW_DB.size
	local adj = 0
	...
	for i=1, #auraList do
		...
		if name then
			if i == 1 then
				local tmp = CreateFrame("frame", "xanAW"..i, UIParent)
				tmp:SetWidth(sWdith)
				tmp:SetHeight(sHeight)
				tmp:SetPoint("TOPLEFT", XAW_Anchor, "BOTTOMRIGHT", 0, 0)
				...
				t:SetWidth(sWdith)
				t:SetHeight(sHeight)
				t:SetPoint("TOPLEFT", XAW_Anchor, "BOTTOMRIGHT", 0, 0)
				...
				tmp:SetWidth(sWdith)
				tmp:SetHeight(sHeight)
				tmp:SetPoint("TOPLEFT", XAW_Anchor, "BOTTOMRIGHT", adj, 0)
				tmp.canCastOther = valChk.canCastOther or false
				tmp.alertChargeNum = valChk.alertChargeNum or nil
				tmp.showTargetName = valChk.showTargetName or nil
				tmp.useRaidWarn = valChk.useRaidWarn or false
				...
				t:SetWidth(sWdith)
				t:SetHeight(sHeight)
				t:SetPoint("TOPLEFT", XAW_Anchor, "BOTTOMRIGHT", adj, 0)
				local g = tmp:CreateFontString("$parentCount", "OVERLAY")
				g:SetFont("Fonts\\FRIZQT__.TTF", 20, "OUTLINE")
				g:SetTextColor( 0.52, 0.96, 0.23)
...
function f:SaveLayout(frame)
	...
	local opt = XanAW_DB[frame] or nil
	...
	for i=1, #auraList do
		...
		if spellname and _G["xanAW"..i] then
			...
			if auraname and caster == "player" then
				...
				_G["xanAW"..i].charges = charges or nil
				if valChk.alertChargeNum and charges and charges <= valChk.alertChargeNum then
					_G["xanAW"..i.."Count"]:SetTextColor( 1, 0, 0)
				else
					_G["xanAW"..i.."Count"]:SetTextColor( 0.52, 0.96, 0.23)
				end
				if valChk.showTargetName then
					_G["xanAW"..i.."TopName"]:SetText(utf8sub(playerName, 1, 5))
				else
					_G["xanAW"..i.."TopName"]:SetText("")
				end
				_G["xanAW"..i].targetN = playerName
				hasShown = true
			...
			if not hasShown and valChk.canCastOther and GetNumRaidMembers() > 0 then
				for q = 1, GetNumRaidMembers() do
					...
					if auraname and unitName ~= playerName and caster == "player" then
						...
						if valChk.showTargetName then
							_G["xanAW"..i.."TopName"]:SetText(utf8sub(unitName, 1, 5))
						else
							_G["xanAW"..i.."TopName"]:SetText("")
						end
						_G["xanAW"..i].targetN = unitName
						hasShown = true
			...
			if not hasShown then
				...
				_G["xanAW"..i.."Count"]:SetText("")
				_G["xanAW"..i.."TopName"]:SetText("")
				_G["xanAW"..i].charges = nil
				_G["xanAW"..i].targetN = nil
			end
